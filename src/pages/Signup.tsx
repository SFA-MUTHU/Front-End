...
const Signup: React.FC = () => {
  ...
  const { loginWithRedirect, isAuthenticated, isLoading, error } = useAuth0(); // Use Auth0 hook
  ...
  const onFinish = async (values: any) => {
    setLoading(true);
    try {
      // Use Auth0's loginWithRedirect with signup screen
      await loginWithRedirect({
        authorizationParams: {
          screen_hint: 'signup', // Trigger signup flow
          login_hint: values.email, // Pre-fill email in Auth0 signup page
        },
        appState: {
          returnTo: '/home',
          userMetadata: {
      ...
      // No need to set loading to false or show toast here
      // as the page will redirect to Auth0
    } catch (error) {
      setLoading(false);
  ...
  if (isLoading) {
    return <div>Loading...</div>; // Replace with a proper loading spinner if desired
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <Form form={form} name="signup_form" layout="vertical" onFinish={onFinish}>
          ...
          <div style={{ textAlign: 'center', marginTop: '16px' }}>
            ...
            <Link to="/login" style={{ color: '#9C7456' }}>
              Login
            </Link>
          </div>
        </Card>
